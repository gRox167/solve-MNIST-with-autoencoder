...
#             best_other, current_label = self.get_best_prototypes(
#                 current_adv, l)
...
#                     self.update_adv(
#                         best_advs[j], current_adv_i, pred_l_i, l_i, x[j])
...
#             current_adv = (1 - coarse_steps) * x + \
#                 coarse_steps * best_advs_imgs_old
...
#                 else:
#                     self.update_adv(
#                         best_advs[j], current_adv[j], pred_l_i, l_i, x[j])
...
#         bs = l.shape[0]
#         p_c, elbos, l_v_classes, reconsts = self.abs.forward(
#             x, return_more=True)
...
class integratedAutoencoder(nn.Module):
    def __init__(self):
        ...
        self.networks = []
        for i in range(10):
            self.networks.append(autoencoder())
        
intAE = integratedAutoencoder()
print(n_mlp.state_dict())
# [ ]change weight for the big model
