...
def test_image_reconstruction(testloader):
    ...
    for net_index in range(1, 11):
        ...
        model.load_state_dict(torch.load(
            './weight/ae_for_number_{}.pth'.format(net_index)))
        for batch_index, batch in enumerate(testloader):
            if(batch_index == 1):
                ...
                outputs1 = model(img)
                # outputs2 = net2(img)
                outputs1 = outputs1.view(
                    outputs1.size(0), 1, 28, 28).cpu().data
                # outputs2 = outputs2.view(outputs2.size(0), 1, 28, 28).cpu().data
                save_image(
                    outputs1, './test/net_{}_batch_{}.png'.format(net_index, batch_index))
...
def test_single_img(testloader):
    """
    Visualize the ae output, takes the first image in each batch, reconstruct with all AEs,
    see output in /test/78output.png
    """
    ...
    for batch_index, batch in (enumerate(tqdm(testloader))):
        ...
        save_image(all_net_output,
                   './test/single_img_input_batch_{}.png'.format(batch_index))
    all_batch_output = all_batch_output.view(
        all_batch_output.shape[0], 1, 28, 28).cpu().data
...
def all_net_output(testloader):
    """
    Test on testset, return the correct fraction
    """
    batch_accuracy = []
    for batch in testloader:
        img, img_label = batch
